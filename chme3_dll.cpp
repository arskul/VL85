  ...
  (const Locomotive *loco,const wchar_t *Type)
{
 ...
 if(!lstrcmpiW(Type,L"chme3")||!lstrcmpiW(Type,L"chme3boost")||!lstrcmpiW(Type,L"chme3t"))
  return true;
...
extern "C" bool __export  CanSwitch
  (const DieselLocomotive *loco,const DieselEngine *eng,
        unsigned int SwitchID,unsigned int SetState)
{
 ...
 }else if((SwitchID>=9&&SwitchID<=14)||SwitchID==17||(SwitchID>=19&&SwitchID<=23)
  ||SwitchID==26
 ){
...
extern "C" void __export Switched
  (const DieselLocomotive *loco,DieselEngine *eng,
        ...
        float time,float AirTemperature)
{
 ...
 if(eng->DieselOn==2){
  ...
  if(eng->FuelConsuming>1.0)
   eng->FuelConsuming=(0.3*eng->Power/993.0)+0.4*(2.0-eng->FuelConsuming);
  else
 ...
 if(!eng->Reverse){
  ...
  if(eng->DieselOn&&eng->ThrottlePosition){
   float VelMin=-3.0+eng->ThrottlePosition*0.225,Vel=loco->Velocity*eng->Reverse;
   float q=Vel-VelMin;
   ...
   Current=0.207*eng->Power*ShuntQ*eng->ThrottlePosition/pow(q,1.85-eng->ThrottlePosition*0.038*ShuntQ1);
   if(Current<ggc)
    Current=0.0;
   else
    Current-=ggc;
   eng->RPM=(eng->Power-InternalPowerUsage)*0.756*(1.0-Current/5.5);
   if(eng->RPM<0.0)eng->RPM=0.0;
 ...
 if(!eng->DieselOn){
  ...
  if(eng->var[11]>12.0)
   Flags&=~16;
  Exhaust(loco,0,1.0,eng->FuelConsuming*60,eng->FuelConsuming*30.0,0xEE9090AA,2.4);
 }else{
  Exhaust(loco,0,0.2+eng->RPM/400.0,eng->FuelConsuming*8.0,eng->FuelConsuming*10.0,0xEE505050,1.0);
 };
 ...
 SMSObject *soundCab=loco->SoundCab(),
           *soundEng=loco->SoundEng();
 if(soundCab){
 ...
 if((State>>8)&1){
  cab->SetDisplayValue(0,loco->Velocity>0?loco->Velocity:-loco->Velocity);
  cab->SetDisplayValue(19,loco->Velocity>0?loco->Velocity:-loco->Velocity);
