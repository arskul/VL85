...
extern "C" bool __export Init
 (DieselEngine *eng,DieselLocomotive *loco,unsigned long State,
...
extern "C" bool __export  CanSwitch(const DieselLocomotive *loco,const DieselEngine *eng,
...
extern "C" void __export Switched(const DieselLocomotive *loco,DieselEngine *eng,
   ...
   switch(SwitchID){
    ...
    case 145:
     if(LocoOn&1){
      st=0; st1=0;
      if(LocoOn&4){
       st=cab->Switch(44); st1=cab->Switch(45);
      };
      if(LocoOn&128){
       st+=cab->Switch(144); st1+=cab->Switch(145);
      };
      if(st)
       Flags|=2;
      else
...
extern "C" void __export ALSN(Locomotive *loco,SignalsInfo *sigAhead,UINT NumSigAhead,
                        SignalsInfo *sigBack,UINT NumSigBack,
                        SignalsInfo *sigPassed,UINT NumPassed,
                        float SpeedLimit, float NextLimit,float DistanceToNextLimit,
                        bool Backwards)
{
 UINT CountGreen=0;
 ...
 if(!Backwards){
  for(UINT i=0;i<NumSigAhead;i++){
   if(sigAhead[i].Flags&1){
    ...
    if(sigAhead[i].Aspect[0]==SIGASP_CLEAR_1 || sigAhead[i].Aspect[0]==SIGASP_CLEAR_2)
     CountGreen++;
    else
     break;
   };
  ...
  for(UINT i=0;i<NumSigBack;i++){
   if(sigBack[i].Flags&1){
    ...
    if(sigBack[i].Aspect[0]==SIGASP_CLEAR_1 || sigBack[i].Aspect[0]==SIGASP_CLEAR_2)
     CountGreen++;
    else
     break;
   };
...
extern "C" void __export Run
 (DieselEngine *eng,const DieselLocomotive *loco,unsigned long State,
        float time,float AirTemperature)
{
 ...
 float Current,Voltage,SetForce,SetBrakeForce,BatteryCharge=0.0,Temp,Temp1;
 ...
 if(eng->DieselOn==3){
  ...
  if(eng->var[1]<0.0&&eng->RPM>300.0){
   eng->DieselOn=1;
   eng->ShowMessage(GMM_HINT,L"Дизель запущен");
  };
 };
 if(eng->DieselOn==2)
  if(eng->RPM>=250.0){
 ...
 if(LocoOn&1){
  ...
  if(LocoOn&2){
   ...
   BatteryCharge=eng->ExternalPowerSource/eng->ExternalPowerSourceVoltage;
   if(BatteryCharge>40.0)
    BatteryCharge=40.0;
  };
 ...
 if(eng->ThrottlePosition){
  ...
  if(Vel<=VelMax)
   ...
   Vel-=VelMax;
   SetForce=10.0*(eng->Power*PowerAvailable*Force1)/pow(Vel,Force2-eng->ThrottlePosition*Force3);
   if(GGc>SetForce)
    SetForce=0.0;
   else
    SetForce-=GGc;
 ...
 if(eng->DieselOn==1){
  ...
  if(SetPower>eng->Power){
   ...
   if(Col>2.5)
    Col=2.5;
   Colour=((char(0xcc+10*Col)&0xFF)<<24)|((char(0x55*Col)&0xFF)<<16)|((char(0x55*Col)&0xFF)<<8)|(char(0x57*Col)&0xFF);
  };
  if(!eng->ThrottlePosition)
   Disp*=0.2;
  Exhaust(loco,0,Rate*eng->Power/1000.0,Mag*eng->Power/1000.0,Disp*eng->Power/500.0,Colour,0);
  Exhaust(loco,1,Rate*eng->Power/1000.0,Mag*eng->Power/1000.0,Disp*eng->Power/500.0,Colour,0);
 }else if(eng->DieselOn==3){
  Exhaust(loco,0,eng->Power/750.0,eng->Power/250.0,eng->Power/80.0,0xdd505060,0);
  Exhaust(loco,1,eng->Power/750.0,eng->Power/250.0,eng->Power/80.0,0xdd505060,0);
 }else{
  ...
  if(loco->LibParam==1&&cab->DisplayState(0)){
   if((LocoOn&1)&&cab->Switch(20)){
    ...
    if(eng->var[15]>0.0 && eng->var[16]>30.0){
     ...
     switch(EPKstate){
      case 1: cab->SetScreenFade(20,0,-1.0,0.8); break;
      case 2: cab->SetScreenFade(20,0,-1.0,0.0); break;
      case 3: cab->SetScreenFade(20,0,-1.0,0.5); break;
      case 4: cab->SetScreenFade(20,0,-1.0,0.3); break;
      case 5: cab->SetScreenFade(20,0,-1.0,0.8); break;
     };
    };
    if(Flags&64)
   ...
   if(eng->DieselOn==1||eng->DieselOn==3){
    ...
    else if(!eng->Force&&(cab->Switch(2)&&cab->Switch(26)&&cab->Switch(20))&&eng->Reverse)
  ...
  if(loco->LibParam==1&&cab->DisplayState(9)){
   if(LocoOn&65){
    ...
    if(eng->var[15]>0.0 && eng->var[16]>30.0){
     ...
     switch(EPKstate){
      case 1: cab->SetScreenFade(21,0,-1.0,0.8); break;
      case 2: cab->SetScreenFade(21,0,-1.0,0.0); break;
      case 3: cab->SetScreenFade(21,0,-1.0,0.5); break;
      case 4: cab->SetScreenFade(21,0,-1.0,0.3); break;
      case 5: cab->SetScreenFade(21,0,-1.0,0.8); break;
     };
    };
    if(Flags&64)
  ...
  switch(loco->LibParam){
   case 1:
    ...
    if(LocoOn&32){
     ...
     if(loco->NumSlaves)
      if(loco->SlaveLoco(0)->BrakeCylinderPressure>0.0 || loco->SlaveLoco(0)->IndependentBrakePressure>0.0)
